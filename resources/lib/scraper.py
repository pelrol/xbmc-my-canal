...
authenticateUrl = "http://service.mycanal.fr/authenticate.json/iphone/1.1?highResolution=1&paired=0&pdsDevice=%5B1%5D"  # noqa: E501
my_canal_ua = "myCANAL/1.0.2 CFNetwork/609.1.4 Darwin/13.0.0"
default_emission_type = "Divertissement \x26 Info"
...
def get(url):
    ...
    # pprint.pprint('url: {}'.format(url))
    response = opener.open(url)

    if response and response.getcode() == 200:
        content = response.read()
    else:
        pprint.pprint("Could not open URL {} to create menu".format(url))
    # pprint.pprint('content: {}'.format(content))
    return content
...
class Emission(object):
    ...
    def get_emission_types(cls):
        ...
        output_json["arborescence"] = dict(
            (arbo["displayName"], arbo) for arbo in output_json["arborescence"]
        )
        ...
        for strate in output_json["strates"]:
            if (
                strate["type"] == "contentRow"
                or strate["type"] == "contentGrid"
            ):
                emission_types.append(
                    {"name": strate["title"].strip(), "index": type_index}
                )
    ...
    def fill_videos_from_content(content, videos):
        ...
        if content["type"] != "landing" and content["type"] != "contentGrid":
            ...
            videos.append(
                {
                    "name": name.strip(),
                    "url": content["onClick"]["URLMedias"].replace(
                        "{FORMAT}", "hls"
                    ),
